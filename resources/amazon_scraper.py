...
def build_amz_search_url(search_term, page_no):
    url = ('https://www.amazon.co.uk/s?k={}&i=toys&rh=p_89%3{}{}&dc&page={}&crid=1SDTYPYXM70YV&qid=1561713015&rnid=1632651031&sprefix=my+li%2Ctoys%2C163&ref=sr_pg_{}').format(
        search_term, 'A', search_term, page_no, page_no)
    return url
...
def get_result_links_from_search_page(page):
    ...
    boxes = soup.find_all(
        class_="s-expand-height s-include-content-margin s-border-bottom")
    for i, item in enumerate(boxes):
        url_list.append('https://www.amazon.co.uk' +
                        item.find(class_="a-link-normal")['href'])
    return url_list
...
def build_review_url(url, page_no):
    review_url = url.replace('dp', 'product-reviews') + \
        '/ref=cm_cr_dp_d_show_all_btm?ie=UTF8&reviewerType=all_reviews&pageNumber=%s' % (
            page_no)
    return review_url
...
def get_links_from_brand_list(brand_list, out_path, max_results_pages):
    ...
    for brand in search_term_list:
        ...
        for page_no in list(range(1, max_results_pages)):
            ...
            search_result_links = get_result_links_from_search_page(
                search_page)
